...
- **Non-Reolink cameras get detections.** This is the one that makes them first-class. A camera added by its RTSP URL had no way to tell Neolink that anything had happened, so event recording, the events page, the timeline's event marks, e-mail and webhook notifications, AI descriptions and Home Assistant's motion sensors were all dead for it — the camera streamed, and that was the end of it. Neolink now subscribes to the camera's own **ONVIF event service** and listens: motion from the standard analytics rules, and person, vehicle or animal where the camera says what it saw. Those become ordinary Neolink events — a clip with a thumbnail, a row on the events page, a notification, a sensor in Home Assistant — indistinguishable from a Reolink camera's. Nothing to configure: if the camera's ONVIF offers events, they start arriving. If it does not, it streams and records around the clock exactly as before, and the switches that need detections stay out of the way rather than pretending. Home Assistant is told again once the subscription lands, so the motion sensors appear without a restart.
- **A detection zone on every camera.** Until now the zone editor only appeared on Reolink cameras with their HTTP API switched on — the grid lives on the camera, so a camera that kept none had nothing to edit. The **Edit zone…** button is now on every camera, and where the zone is kept depends on what the camera can do. A Reolink that holds its own grid is completely unchanged: you are still editing the camera's zone, at the dimensions its firmware reports, and it still governs the camera's own motion and AI alarms. Every other camera — a generic RTSP one, and Reolink models whose firmware carries no grid — gets a zone that **Neolink keeps on its behalf**, which the editor says in as many words: nothing is written to the camera, its own alerts are untouched, and what the zone governs is what Neolink watches, which today is the live object boxes drawn over a camera you are looking at. The grid is sized to the picture (32×18 on a 16:9 camera), starts out watching everywhere, is stored next to the other per-camera settings and survives a restart.
- **A still picture for cameras that have no snapshot command.** A generic RTSP camera has no way to be asked for a photograph, so it had none anywhere: no thumbnail on its wall tile, and nothing to draw a detection zone over. Neolink now takes one out of the video it is already carrying — one frame, decoded with ffmpeg — so the zone editor has the real scene behind its grid and the tiles have a poster. It happens only for cameras that have no snapshot command of their own: a Reolink whose snapshot fails is offline, asleep or busy, and it answers exactly as it always did rather than starting to decode video. With no ffmpeg installed, a camera without a snapshot command simply has no still, as before.
- **Two interoperability fixes that decide whether a camera works at all.** Requests to a camera are now stamped in **its own clock**, read from it first (the one ONVIF call that needs no login, precisely so it can be asked before the clocks are known to agree) — a camera drifting from your server by more than a few seconds rejects every request otherwise, and the symptom is an empty settings panel that looks exactly like a wrong password. And the camera's login is now offered over **HTTP authentication as well as WS-Security**, because which of the two a given firmware insists on is not something you can tell from the outside. Stills come from the camera's own **ONVIF snapshot** where it offers one, rather than decoding a video frame — cheaper, and it works without ffmpeg.
- **Zoom, newer cameras, and a zone the camera itself obeys.** Three more of the ONVIF standard, for non-Reolink cameras. A camera with a motorised lens gets an **optical zoom slider**; a lens that zooms but cannot pan or tilt no longer shows arrows that would do nothing. Cameras that speak only **Media2** — ONVIF's newer media service, which some recent firmware offers instead of the old one — now get their streams, stills and overlays too; the old service is still asked first, so cameras that answer it (every Reolink) see no difference. And where a camera runs ONVIF's standard **cell motion detector** — Hikvision, Dahua, Axis and many others do — the detection zone you draw is now **written to the camera**, at the camera's own grid size, and decides where its motion alarms fire. Only the cells change: how many must move and the alarm delays stay as the camera had them, and its other analytics rules keep their own areas. A camera without that detector keeps its zone on Neolink exactly as before.
- **Settings for non-Reolink cameras, over ONVIF.** A camera added by its RTSP URL used to stream and record and nothing else — its settings tab was not even offered. It now gets the same panel the Reolink cameras get, built from whatever the camera's own **ONVIF** services answer for: its make, model, firmware and serial on the identity strip; its stream profiles' resolution, framerate and bitrate, changeable; the picture sliders with day/night; pan and tilt with its saved positions, and a way to save a new one; where the name and timestamp overlays sit; and reboot. Each section appears only if that camera actually offers it. What ONVIF cannot do is left out rather than offered and then refused — it can move an overlay but not switch one off, and it has no hue, anti-flicker, flip/mirror or HDR. Turn ONVIF on in the camera's own web page and Neolink finds it on the stream URL's host, signing in with the login that URL carries; `onvif_address` points it somewhere else, and can carry its own login for cameras that keep separate accounts for streaming and management. **Test connection** now tells you whether ONVIF answered while you are still adding the camera. A camera with ONVIF off streams and records exactly as before.
...
- **The desktop app's dialogs no longer clip on scaled displays.** The
  ...
  code with no designer baseline, never actually scaled — on a 125% or 150%
  display the text grew into fixed pixel sizes and buttons fell off the
  bottom. Both now scale with the font from an explicit baseline, and the
  connect dialog got room for its longer first-run guidance.
...
- **Opus audio for RTSP clients — chosen per client, by URL.** Any camera URL
  ...
  AAC/ADPCM transcoded to Opus — what WebRTC ecosystems (go2rtc, browsers,
  Home Assistant WebRTC cards) take natively, saving them their own ffmpeg
  hop — `original` forces the camera's own track, so an NVR records AAC and a
  WebRTC gateway pulls Opus from the same camera at the same time.
  `"audio_transcode": "opus"` on a camera flips its bare URL's default. The
  transcode rides the same optional ffmpeg the AI features use (libopus,
  which the standard builds all carry), runs only while an Opus client is
  actually connected, and costs well under 1% of a core; without ffmpeg —
  and for any unsupported format — the request 404s with a log line saying
  why. Recordings, the web UI player and two-way talk always keep the
  camera's original audio.
- **AUDIO section in camera settings.** The speaker-volume section grows into
  ...
  **record audio** switch (the camera-side flag that puts the microphone into
  ...
  camera setting, always read from the camera itself. The record-audio switch
  is also announced to Home Assistant (per camera, only where the firmware
  exposes it) so automations can mute or unmute recordings.
