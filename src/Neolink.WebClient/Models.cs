    ...
    bool AiPending = false)
{
    ...
    /// a DIFFERENT URL than the finished clip — never shadow it.</summary>
    public string ArtifactVersion => Ongoing ? "live" : End.Ticks.ToString();
...
public static class UiIcon
{
    public static MarkupString Render(string name, int size = 15)
    {
        var body = name switch
        {
            ...
            // Speaker with sound waves; the -x variant is the muted twin.
